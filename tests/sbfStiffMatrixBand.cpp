...
{
    std::unique_ptr<sbfMesh> m_up ( sbfMesh::makeBlock ( 100.0, 1.0, 1.0, 4, 2, 1, ElementType::HEXAHEDRON_LINEAR ) );
    sbfMesh *m = m_up.get();
    ...
    stiff->lockDof(m->nodeAt(0, 0, 0), 0, 0, force.data(), LockType::EXACT_LOCK_TYPE);
    stiff->lockDof(m->nodeAt(0, 0, 0), 1, 0, force.data(), LockType::EXACT_LOCK_TYPE);
    stiff->lockDof(m->nodeAt(0, 0, 0), 2, 0, force.data(), LockType::EXACT_LOCK_TYPE);
    stiff->lockDof(m->nodeAt(0, 1, 0), 0, 0, force.data(), LockType::EXACT_LOCK_TYPE);
    stiff->lockDof(m->nodeAt(0, 1, 0), 2, 0, force.data(), LockType::EXACT_LOCK_TYPE);
    stiff->lockDof(m->nodeAt(0, 0, 1), 0, 0, force.data(), LockType::EXACT_LOCK_TYPE);
...
/*        std::vector<std::vector<double>> data;
        data.resize(m->numNodes()*3);
        for(auto &d : data) d.resize(m->numNodes()*3);
