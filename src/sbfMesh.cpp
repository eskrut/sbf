...
int sbfMesh::readCrdFromFile(const char* crdName)
{
    ...
    if(!crdFile.good())
    {
        std::cout << "Error! Can't read crd file " << crdName << std::endl;
        return 1;
    }
...
int sbfMesh::readIndFromFile(const char* indName, FileVersion fileFormatVersion)
{
    ...
    if(!indFile.good()){
        std::cout << "Error! Can't read ind file " << indName << std::endl;
        return 1;
    }
    ...
    else {std::cout << "Ind file version error" << std::endl; return 1;}
    indFile.close();
...
int sbfMesh::readMtrFromFile(const char* mtrName)
{
    ...
    if(!mtrFile.good())
    {
        std::cout << "Error! Can't read mtr file " << mtrName << std::endl;
        return 1;
    }
...
int sbfMesh::writeCrdToFile(const char* crdName)
{
    ...
    if(!crdFile.good())
    {
        std::cout << "Error! Can't open crd file " << crdName << " for writing" << std::endl;
        return 1;
    }
...
int sbfMesh::writeIndToFile(const char* indName, const FileVersion fileFormatVersion)
{
    ...
    if(!indFile.good())
    {
        std::cout << "Error! Can't open ind file " << indName << " for writing" << std::endl;
        return 1;
    }
...
int sbfMesh::writeMtrToFile(const char* mtrName, const FileVersion fileFormatVersion) const
{
    ...
    if(!mtrFile.good())
    {
        std::cout << "Error! Can't open mtr file " << mtrName << " for writing" << std::endl;
        return 1;
    }
    if(fileFormatVersion == FileVersion::OLD_FORMAT){
        for(int ct = 0; ct < numElements(); ct++){
        ...
        if(numGroups() == 0)
            std::cout << "Mtr file write failed! No groups!" << std::endl;
        else{
            int nTypes = numGroups();
            for(int ct = 0; ct < nTypes; ct++){
                for(int ct1 = 0; ct1 < elemGroups_[ct]->numElems(); ct1++){
...
void sbfMesh::printInfo()
{
    ...
    std::cout << "Elements in mesh:" << std::endl;
    std::cout << "Beam 3 DOF linear\t\t" << numElementsMap[ElementType::BEAM_LINEAR_3DOF] << std::endl;
    std::cout << "Beam 3 DOF quadratic\t\t" << numElementsMap[ElementType::BEAM_QUADRATIC_3DOF] << std::endl;
    std::cout << "Beam 6 DOF linear\t\t" << numElementsMap[ElementType::BEAM_LINEAR_6DOF] << std::endl;
    std::cout << "Beam 6 DOF quadratic\t\t" << numElementsMap[ElementType::BEAM_QUADRATIC_6DOF] << std::endl;
    std::cout << "Hexahedron linear\t\t" << numElementsMap[ElementType::HEXAHEDRON_LINEAR] << std::endl;
    std::cout << "Hexahedron quadratic\t\t" << numElementsMap[ElementType::HEXAHEDRON_QUADRATIC] << std::endl;
    std::cout << "Tetrahedron linear\t\t" << numElementsMap[ElementType::TETRAHEDRON_LINEAR] << std::endl;
    std::cout << "Tetrahedron quadratic\t\t" << numElementsMap[ElementType::TETRAHEDRON_QUADRATIC] << std::endl;
    std::cout << "Undefined elements\t\t" << numElementsMap[ElementType::NO_DEFINED_ELEMENT] << std::endl;
    std::cout << "Number of elements groups " << this->numGroups() << std::endl;
    std::cout << "Number of nodes groups " << this->numNodeGroups() << std::endl;
    std::cout << "Bounding box:" << std::endl << "xMin = " << minX() << "\txMax = " << maxX() << std::endl << "yMin = " << minY() << "\tyMax = " << maxY() << std::endl << "zMin = " << minZ() << "\tzMax = " << maxZ() << std::endl;
}
...
void sbfMesh::optimizeNodesNumbering(RenumberOptimizationType type)
{
    ...
    std::cout << "Until number optimization: size = " << size << " bandwidth = " << computeBandwidth(graph, numNodes) << std::endl;
    ...
    computeGraphAlter(this, &graph);
    size = computeProfileSize(graph, numNodes);
    std::cout << "After number optimization: size = " << size << " bandwidth = " << computeBandwidth(graph, numNodes) << std::endl;
...
void computeGraph(sbfMesh * mesh, int *** graph)
{
    ...
    for(int curNode = 0; curNode < numNodes; curNode++){//Loop on nodes
        if(curNode % (numNodes/20 > 0 ? numNodes/20 : 1) == 0)
            std::cout << "\r" << curNode << " of " << numNodes << " (" << (curNode+1)*100/numNodes << "%)";
        numMask = 0;
...
void makeRCM(int ** graph, int numNodes, int startNode, int * newToOld, int *oldToNew)
{
    ...
    for(curNode = 0; curNode < numNodes - 1; curNode++){//Loop on nodes
        ...
        if(newToOld[curNode + 1] == -1){
            std::cout << "makeRCM error newToOld[" << curNode + 1 << "] == -1, tmp = " << count << std::endl;
            break;
        }
    }//Loop on nodes
