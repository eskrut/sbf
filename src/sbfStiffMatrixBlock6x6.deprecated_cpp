...
sbfStiffMatrix *sbfStiffMatrixBlock6x6::createChol()
{
    ...
    for(int ct = 0; ct < mesh_->numElements(); ++ct) elementIDs.push_back(ct);
    cholFactor->numNodes_ = cholFactor->mesh_->numNodes();
    indexes.resize(cholFactor->numNodes_);
    for(auto ind : elementIDs) {
        std::vector<int> nodeIDs = cholFactor->mesh_->elemPtr(ind)->indexes();
        for(auto node1 : nodeIDs) for(auto node2 : nodeIDs) indexes[node1].insert(node2);
    }
    for(auto &row : indexes) {
        int first = *row.begin();
        int last = *row.rbegin();
        for(int ct = first; ct <= last; ++ct) row.insert(ct);
    }
    int indLength = 0;
    for(auto &row : indexes) indLength += row.size();

    if( cholFactor->type_ & MatrixType::FULL_MATRIX) {
        cholFactor->setNumBlocksNodes(indLength, cholFactor->numNodes_, 0);
        int count = 0;
        int ct = 0;
        shiftInd_[count++] = ct;
        for(auto &row : indexes) {
            for(auto nodeID : row)
                cholFactor->indJ_[ct++] = nodeID;
            cholFactor->shiftInd_[count++] = ct;
        }
    }//MatrixType::FULL_MATRIX
    else if(cholFactor->type_ & UP_TREANGLE_MATRIX || cholFactor->type_ & DOWN_TREANGLE_MATRIX){
        int numBeforeDiagonal = 0, numAfterDiagonal = 0;
        int size = static_cast<int>(indexes.size());
        for(int ct = 0; ct < size; ++ct) {
            auto &row = indexes[ct];
...
//        //TODO is it true?
//        assert(numBeforeDiagonal == numAfterDiagonal);
//        assert(numBeforeDiagonal == (indLength-cholFactor->numNodes_)/2);
        if (cholFactor->type_ & UP_TREANGLE_MATRIX) cholFactor->numBlocksAlter_ = numBeforeDiagonal;
        else cholFactor->numBlocksAlter_ = numAfterDiagonal;
        cholFactor->numBlocks_ = indLength - cholFactor->numBlocksAlter_;
        cholFactor->setNumBlocksNodes(cholFactor->numBlocks_, cholFactor->numNodes_, cholFactor->numBlocksAlter_);
        int countNorm = 0;
        int ctNorm = 0;
        int countAlter = 0;
        int ctAlter = 0;
        cholFactor->shiftInd_[countNorm++] = ctNorm;
        cholFactor->shiftIndAlter_[countAlter++] = ctAlter;
        size = static_cast<int>(indexes.size());
        for(int ct = 0; ct < size; ++ct) {
            auto &row = indexes[ct];
            for(auto nodeID : row) {
                if (cholFactor->type_ & UP_TREANGLE_MATRIX) {
                    if (nodeID >= ct) cholFactor->indJ_[ctNorm++] = nodeID;
                    else {cholFactor->indJAlter_[ctAlter] = nodeID; cholFactor->ptrDataAlter_[ctAlter] = nullptr; ctAlter++;}
                }
                else {
                    if (nodeID <= ct) cholFactor->indJ_[ctNorm++] = nodeID;
                    else {cholFactor->indJAlter_[ctAlter] = nodeID; cholFactor->ptrDataAlter_[ctAlter] = nullptr; ctAlter++;}
